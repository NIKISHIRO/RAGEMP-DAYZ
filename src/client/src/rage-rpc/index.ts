...
if(!glob[PROCESS_EVENT]){
    ...
    glob[PROCESS_EVENT] = (player: Player | string, rawData?: string) => {
        ...
        const data: any = util.parseData(rawData);
...
function passEventToBrowser(browser: Browser, data: any, ignoreNotFound: boolean): void {
...
function callEvent(name: string, args: any, info: ProcedureListenerInfo){
    ...
    if(listeners){
        listeners.forEach((listener: (arg0: any, arg1: ProcedureListenerInfo) => any) => listener(args, info));
    }
}
